...
export interface State { leads:Lead[]; memory:MemoryItem[]; skills:SkillVersion[]; audit:AuditEvent[]; approvals:Approval[]; missions:Mission[]; activities:Activity[]; revenue:RevenueRecord[]; researchQueue:Lead[]; businesses:BusinessIdea[]; strategyDecisions:StrategyDecision[]; settings:{paused:boolean; emergencyStop:boolean}; }
const empty:State={leads:[],memory:[],skills:[],audit:[],approvals:[],missions:[],activities:[],revenue:[],researchQueue:[],businesses:[],strategyDecisions:[],settings:{paused:false,emergencyStop:false}};
export class JsonStore {
  constructor(private file=process.env.DATA_FILE||'data/tw01.json'){}
  async load():Promise<State>{
    try{
      const raw=JSON.parse(await readFile(this.file,'utf8'));
      return {leads:raw.leads||[],memory:raw.memory||[],skills:raw.skills||[],audit:raw.audit||[],approvals:raw.approvals||[],missions:raw.missions||[],activities:raw.activities||[],revenue:raw.revenue||[],researchQueue:raw.researchQueue||[],businesses:raw.businesses||[],strategyDecisions:raw.strategyDecisions||[],settings:{...empty.settings,...(raw.settings||{})}};
