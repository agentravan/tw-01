...
function renderDetail(id){
 ...
 <h3>Run history</h3>${runs.length?runs.map(r=>`<div class="runRow"><b>${esc(r.status)}</b> · ${new Date(r.startedAt).toLocaleString('en-IN')} · ${r.durationMs?Math.round(r.durationMs/1000)+'s':'active'}<small> Run ${esc(r.id.slice(0,8))} · heartbeat ${new Date(r.heartbeatAt).toLocaleTimeString('en-IN')} · tools ${r.toolCalls} · cost $${r.costUsd}</small></div>`).join(''):'No runs yet.'}`;
}
